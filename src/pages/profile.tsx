...
const Profile: React.FC = () => {
    ...
    const [portfolio, setPortfolio] = useState<{ cryptocurrency_id: number; amount: number }[]>([]);
    ...
    useEffect(() => {
        const fetchCryptoPrice = async () => {
            const selected = cryptocurrencies.find(c => c.id === selectedCoinId);
            if (!selected) return;

            try {
                const response = await axios.get(`https://api.coingecko.com/api/v3/simple/price`, {
                    params: {
                        ids: selected.symbol.toLowerCase(),
                        vs_currencies: 'usd',
                    },
                });
                setCryptoPrice(response.data[selected.symbol.toLowerCase()]?.usd || null);
            } catch (error) {
        ...
        fetchCryptoPrice();
    }, [selectedCoinId, cryptocurrencies]);
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 p-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-md w-full max-w-2xl mt-4">
                ...
                <ul>
                    {portfolio.length > 0 ? (
                        portfolio.map((entry) => {
                            const coin = cryptocurrencies.find(c => c.id === entry.cryptocurrency_id);
                            return (
                                <li key={entry.cryptocurrency_id} className="flex justify-between border-b py-2">
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50">
                    <div className="bg-white p-6 rounded-lg shadow-md w-96">
                        ...
                        <input
                            ...
                            placeholder="Ilość"
                        />
                        <p className="mt-2">{transactionType === 'buy' ? 'Koszt transakcji' : 'Wartość transakcji'}: {cryptoPrice ? `$${(amount * cryptoPrice).toFixed(2)}` : 'Ładowanie...'}</p>
